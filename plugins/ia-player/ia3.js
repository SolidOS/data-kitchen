...
function migrateConfigUrl(url) {
  ...
  let u = url.replace('/ia-music-library/', '/plugins/ia-player/libraries/internet_archive_music/');
  if (u === './ia-music.ttl' || u.endsWith('/ia-music.ttl')) {
    u = u.replace(/(^|\/)ia-music\.ttl$/,
      (_, p) => `${p}plugins/ia-player/libraries/internet_archive_music/index.ttl`);
  }
  // libraries-layout era (pre-plugins) pointers — skip URLs already migrated:
  if (!u.includes('/plugins/ia-player/libraries/')) {
    u = u.replace(/(^|\/)libraries\/(internet_archive_music|internet_archive_movies)\//,
      (_, p, lib) => `${p}plugins/ia-player/libraries/${lib}/`);
  }
  return u;
}

 ...
 * ui:name. Run after updating sol-components. Like seed-plugins-catalog,
...
function enrich(dir) {
  ...
  for (const f of readdirSync(dir).filter((x) => x.endsWith('.ttl'))) {
    ...
    // Match by ui:name; a launcher entry (e.g. calendar's <sol-button
    // data-handler="sol-calendar">) matches by its data-handler value —
    // the same key dk-plugin-settings uses for the settings component.
    const name = /ui:name\s+"([^"]+)"/.exec(ttl)?.[1];
    const handler = /schema:name\s+"data-handler"\s*;\s*schema:value\s+"([^"]+)"/.exec(ttl)?.[1];
    const key = canonical.has(name) ? name : handler;
    ...
    // insert after the ui:name line (every dk component ttl has one)
    ttl = ttl.replace(/^(\s*ui:name\s+"[^"]+"\s*;\s*)$/m, `$1\n${lines.join('\n')}`);

#!/usr/bin/env node
/**
 * rdf2html — emit an editable HTML snapshot of the shell from data/tabs.ttl.
 *
 * The shell is rdf-first: data/tabs.ttl is the only live artifact (rendered at
 ...
 *   (edit the snapshot)
 *   npm run html2rdf            # import the edits back into data/tabs.ttl
 *
...
const BANNER = `<!-- Editable HTML snapshot of the dk shell, generated from data/tabs.ttl by
     ...
     html2rdf does not import it; edit data/tabs.ttl#Chrome directly. -->
`;
...
if (VERIFY) {
  ...
  console.log(same ? 'VERIFY OK — snapshot matches data/tabs.ttl'
                   : 'VERIFY MISMATCH — snapshot differs from what data/tabs.ttl generates');
